...
def update_db_with_crawled_data(vector_store: DB, documents: list[Document], ts_orphaned: int) -> None:
    """Update the database with new crawled data."""

    data_add, data_update_meta, data_del = compare_crawled_data_with_db(vector_store, documents)
    ...
    vector_store.update_metadata(data_update_meta)

    # Delete orphaned objects
    vector_store.delete_orphaned(ts_orphaned)
...
def compare_crawled_data_with_db(vector_store: DB, data: list[Document]) -> tuple[list[Document], list[Document], list[Document]]:
    ...
    """
    data_add, data_delete, data_update_meta = [], [], []
    for d in data:
    ...
    return data_add, data_update_meta, data_delete


async def update_db_with_crawled_data_using_internal_cache(
    vector_store: VectorStore, documents: list[Document], cache_key_name: str, orphaned_days: float
) -> None:
    """
    ...
    documents older than the specified 'orphaned_days' are removed.
    """
    ...
    chunks_to_delete, chunks_old_keep = get_chunks_to_delete(previous_runs, documents, orphaned_days=orphaned_days)

    Actor.log.info("Chunks to add: %s, chunks to update metadata: %s", len(chunks_to_add), len(chunks_to_update))
