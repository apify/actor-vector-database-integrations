...
db = WeaviateVectorStore(client=weaviate_client, index_name=WEAVIATE_COLLECTION_NAME, text_key="text", embedding=embeddings)
...
coll = weaviate_client.collections.get(name=WEAVIATE_COLLECTION_NAME)

all_objects = [o for o in coll.iterator()]
print("All objects in the collection", len(all_objects), all_objects)

ids = [str(o.uuid) for o in all_objects]
print("All object ids in the collection", len(ids), ids)
...
if DROP_AND_INSERT:
    # db.delete_all()
    # Insert objects
    inserted = db.add_documents(documents=crawl_1, ids=[d.metadata["id_"] for d in crawl_1])
